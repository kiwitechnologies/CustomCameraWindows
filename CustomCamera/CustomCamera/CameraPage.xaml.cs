...
namespace CustomCamera
{
    ...
    public sealed partial class CameraPage : Page
    {
        ...
        {
            stkFlashButton.Visibility = Visibility.Collapsed;
            stkZoomButton.Visibility = Visibility.Collapsed;
            stkFocusButton.Visibility = Visibility.Collapsed;
            stkWbButton.Visibility = Visibility.Collapsed;
            stkIsoButton.Visibility = Visibility.Collapsed;
            stkShutterButton.Visibility = Visibility.Collapsed;
            stkEvButton.Visibility = Visibility.Collapsed;
            await SetupUiAsync();
        ...
        private async Task TakePhotoAsync()
        {
            ...
            try
            {
                ...
                Frame.Navigate(typeof(Crop), stream);
            }
            catch (Exception ex)
            {
        ...
        {
            if(grdImage.Visibility == Visibility.Visible)
            {
        ...
        private void SetSingleControl(object activeButton)
        {
            ...
            var activeResult = activeButton as AppBarButton;
            if (activeResult != null)
            {
                strButtonName = activeResult.Name;
            }
            ...
            {
                if (!button.Name.Contains(strButtonName)) // != activeButton)
                {
                ...
                {
                    button.Visibility = _singleControlMode ? Visibility.Visible : Visibility.Collapsed;
                }
            }
        ...
        {
            var point = Point2;
            if (e.Position.X <= imgSource.ActualWidth && e.Position.X >= 0)
                point.X = e.Position.X;
            if (e.Position.Y <= imgSource.ActualHeight && e.Position.Y >= 0)
        ...
        private async void CropBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            BitmapImage bImage = imgSource.Source as BitmapImage;
            ...
            using (IRandomAccessStream streamWithContent = await storageFile.OpenAsync(FileAccessMode.Read))
            {
                ...
                double displayedWidth = imgSource.ActualWidth;
                double displayedHeight = imgSource.ActualHeight;
